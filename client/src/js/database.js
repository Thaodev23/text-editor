...
export const putDb = async (content) => {
  ...
  // TODO: Change text to whatever variable name you wanT. Note that you'll then need to change any other occcurences of text to the same variable name.
  const text = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const tx = text.transaction(DB_NAME, 'readwrite');
...
export const getDb = async () => {
  ...
  const text = await openDB(DB_NAME, 1);
  const tx = text.transaction(DB_NAME, 'readonly');
  const store = tx.objectStore(DB_NAME);
